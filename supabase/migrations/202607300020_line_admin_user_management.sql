...
as $$
    ...
    select
      ...
      left(
        coalesce(
          ...
          nullif(
            btrim(auth_user.raw_user_meta_data ->> 'name'),
            ''
          ),
          nullif(
            btrim(
  ...
  select
    ...
    case
      ...
      when sanitized.email is not null
        and sanitized.email_confirmed_at is null
        then 'unconfirmed'
      else 'active'
    end as status,
  ...
  where (
      ...
      or sanitized.display_name ilike
        '%' || btrim(p_search_text) || '%'
      or sanitized.user_id::text ilike
        '%' || btrim(p_search_text) || '%'
    )
    and (
...
create or replace function public.purge_private_user_data(
...
declare
  ...
  v_confirmation text := btrim(p_normalized_email);
begin
  if v_confirmation is null or v_confirmation = '' then
  ...
  if v_existing.id is not null then
    if v_existing.target_user_id <> p_target_user_id
      ...
      or coalesce(
        v_existing.details ->> 'confirmation',
        v_existing.details ->> 'email',
        ''
      ) <> v_confirmation
    then
      raise exception using
